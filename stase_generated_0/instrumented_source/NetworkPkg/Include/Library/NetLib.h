...
#define   NET_POOL_SIGNATURE  SIGNATURE_32 ('n', 'p', 'w', 'l')

#define   NET_POOL_BUCKETS    0x20
#define   NET_POOL_MINIMUM    0x20
#define   NET_CACHE_COUNT     0x18

//
// A few pointer-math helpers
//
#define   ALIGN_NET_POOL(x) (((x) + 15) & ~15)
#define   NET_POOL_INDEX(x) (((x) >> 4) - 1)
#define   NET_INDEX_LEN(x) (((x) + 1) << 4)

//
// Flags for the NET_POOL_HEAD
//
#define   NET_POOL_FROM_BUCKET  0x01
#define   NET_POOL_FROM_BASE    0x02
#define   NET_POOL_NET_FREE     0x04
#define   NET_POOL_BASE_FREE    0x08

typedef struct _NET_POOL_HEAD {
  UINT32                   PoolFlags;      // NetPool description flags
  UINT32                   PoolLen;        // NetPool Data length
  struct _NET_POOL_HEAD   *Next;           // Pointer to the next in list
  CHAR8                   Data[];
} NET_POOL_HEAD;

#define   NET_POOL_SIZE(x)  (((x) + OFFSET_OF(NET_POOL_HEAD, Data)))
...
/**
  Allocate a pool from the NET_POOL cache. Initial the memory block with NULL.
...
typedef VOID* (EFIAPI *NET_POOL_ALLOCATE)(IN UINT32 Len);
typedef VOID (EFIAPI *NET_POOL_FREE)(IN VOID* PoolData);

typedef struct {
  UINT32                Signature;
  NET_POOL_FREE         Free;
  NET_POOL_ALLOCATE     Allocate;
  NET_POOL_ALLOCATE     AllocateZero;
  NET_POOL_HEAD         *Bucket[NET_POOL_BUCKETS];
} NET_POOL_PRIVATE;
