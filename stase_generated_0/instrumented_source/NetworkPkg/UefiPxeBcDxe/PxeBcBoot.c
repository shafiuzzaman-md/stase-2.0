...
/**
  Allocates and initializes a reply object for network boot operations.
  ...
  @param[in]  Handle      A pointer to the EFI_HANDLE.
  @param[in]  CBoot       Pointer to the custom PXE boot structure where the 
                          reply object will be stored.

  @retval EFI_SUCCESS            The reply object was successfully created and initialized.
  @retval EFI_OUT_OF_RESOURCES   Memory allocation failed.

**/
EFI_STATUS 
PxeBcCreateReplyObject (
  IN EFI_GUID *Guid, 
  IN EFI_HANDLE *Handle, 
  IN PXEBC_BOOT_CUSTOM *CBoot
  )
{
  CBoot->ReplyObject = (NETWORK_OBJECT *)AllocateZeroPool(sizeof(NETWORK_OBJECT));
  if (CBoot->ReplyObject == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  CBoot->ReplyObject->Body.ContentLength = 1024;
  CBoot->ReplyObject->Body.ContentData = (CHAR16 *)AllocateZeroPool(1024 * sizeof(CHAR16));
  if (CBoot->ReplyObject->Body.ContentData == NULL) {
    FreePool(CBoot->ReplyObject);
    return EFI_OUT_OF_RESOURCES;
  }

  // Initialize with reply data
  StrnCpyS(CBoot->ReplyObject->Body.ContentData,  1024 * sizeof(CHAR16), L"Default network boot reply",  1024 * sizeof(CHAR16));
  return EFI_SUCCESS;
}

/**
  Increments the ReferenceCount of a network object.

  @param[in]  Object  Pointer to the body of the network object.

**/
VOID
ObReferenceObject (
  NETWORK_OBJECT_BODY *Object
  )
{
  NETWORK_OBJECT_HEADER *header = (NETWORK_OBJECT_HEADER *)((UINT8 *)Object - OFFSET_OF(NETWORK_OBJECT, Body));
  header->ReferenceCount++;
}

/**
  Processes a network boot response by incrementing the reference 
  count and potentially simulating network activity.

  @param[in]  CBoot  Pointer to the custom PXE boot structure.

**/
VOID 
PxeBcVendorBootResponse (
  IN PXEBC_BOOT_CUSTOM *CBoot
  )
{
  NETWORK_OBJECT_BODY *rObjectBody;

  if (!CBoot->ReplyRequested) {
		CBoot->LastOperationStatus = EFI_NO_RESPONSE;
    return;
	}
	rObjectBody = &CBoot->ReplyObject->Body;
	ObReferenceObject(rObjectBody);

  // Network info log
  if (CBoot->NetworkInfo.Port != 0) {
    DEBUG((DEBUG_INFO, "Reply to %s on port %d\n", CBoot->NetworkInfo.MacAddress, CBoot->NetworkInfo.Port));
  }

  CBoot->LastOperationStatus = EFI_SUCCESS;  // Update status
}
