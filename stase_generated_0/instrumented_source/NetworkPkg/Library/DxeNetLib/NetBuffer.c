  ...
  )
{
  ...
  //
  Nbuf = AllocateZeroNetPool (NET_BUF_SIZE (BlockOpNum));
  ...
  if (BlockNum != 0) {
    Vector = AllocateZeroNetPool (NET_VECTOR_SIZE (BlockNum));
  ...
  )
{
  ...
  if (Vector->Free != NULL) {
    if ((Vector->Flag & NET_VECTOR_OWN_FIRST) != 0) {
      FreeNetPool (Vector->Block[0].Bulk);
    }
    ...
    for (Index = 0; Index < Vector->BlockNum; Index++) {
      FreeNetPool (Vector->Block[Index].Bulk);
    }
  }
  ...
  )
{
  ...
  Clone = AllocateNetPool (NET_BUF_SIZE (Nbuf->BlockOpNum));
  ...
  )
{
  ...
  if (HeadSpace != 0) {
    ...
    FirstBulk = AllocateNetPool (HeadSpace);
  ...
  )
{
  ...
  if ((HeadSpace != 0) || (HeadLen != 0)) {
    ...
    FirstBlock    = AllocateNetPool (FirstBlockLen);
  ...
  )
{
  ...
  //
  Fragment = AllocateNetPool (sizeof (NET_FRAGMENT) * FragmentNum);
  ...
  )
{
  ...
  NbufQue = AllocateNetPool (sizeof (NET_BUF_QUEUE));
  if (NbufQue == NULL) {
    return NULL;
  }
  ...
  )
{
  ...
  if (Nbuf->RefCnt == 0) {
    ...
    if ((Nbuf->Vector->Flag & NET_VECTOR_OWN_FIRST) != 0) {
      FreeNetPool (Nbuf->Vector->Block[0].Bulk);
    }

    FreeNetPool (Nbuf->Vector);
    FreeNetPool (Nbuf);
  }
}
