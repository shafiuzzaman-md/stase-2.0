...
//                                      );
  if (Attribute == 0x00 || EFI_ERROR (Status)) {
    ...
    klee_assert(((Attribute & EFI_MEMORY_XP) != 0));
...
MakeDispatch(
  IN      EFI_SMM_LEGACY_CONTEXT            *CpuContext,
  IN      EFI_SMM_LEGACY_DISPATCH_CONTEXT   *DispatchContext,
  IN      EFI_HANDLE                        *DispatchHandle,
  IN      EDKII_SMM_LEGACY_REGISTER_CONTEXT *RegContext,
  ...
  )
{
  ...
  if ((ConfigurationFlags & 0x4) && EFI_ERROR (Status)) {
    DEBUG((DEBUG_INFO, "%a: Smm Legacy dispatch fail (0x%08x)\n", __func__, CpuContext->CommandPort));
  }

  if ((ConfigurationFlags & 0x2) == 0) {
    return (EFI_SMM_LEGACY_DISPATCH_CONTEXT *)NULL;
  }
...
//   Status                  = GetNvConfiguration();
  ...
  //
  // These will never change between dispatches
  //
  Size                    = *CommBufferSize;
...
//   FindContextBySwSmiInputValue (CpuContext.CommandPort, &DispatchContext);\
  while (DispatchContext != (EFI_SMM_LEGACY_DISPATCH_CONTEXT *)NULL) {
    DispatchContext =
      MakeDispatch (&CpuContext, DispatchContext, &DispatchHandle, &RegContext, CommBuffer, &Size);
//   }
