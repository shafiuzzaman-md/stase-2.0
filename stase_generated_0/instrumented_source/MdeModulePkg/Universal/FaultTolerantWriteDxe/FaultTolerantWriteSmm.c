...
#include "../../Include/Protocol/SmmSwapAddressRange.h"
#include "FaultTolerantWrite.h"
#include "../../../MdePkg/Include/Protocol/MmEndOfDxe.h"
  ...
  )
{
  ...
  CopyMem((void *)&FtwArgs.SmmFtwFunctionHeader, CommBuffer, SMM_FTW_COMMUNICATE_HEADER_SIZE);
  FtwArgs.StructureData = (void *)((SMM_FTW_COMMUNICATE_FUNCTION_HEADER *)CommBuffer)->Data;
  ...
  if (mEndOfDxe) {
    ...
    //
    DEBUG ((DEBUG_ERROR, "SmmFtwHandler: Not safe to do the operation: %x after End Of Dxe, so access denied!\n", FtwArgs.SmmFtwFunctionHeader.Function));
    FtwArgs.SmmFtwFunctionHeader.ReturnStatus = EFI_ACCESS_DENIED;
    return EFI_SUCCESS;
  }

  switch (FtwArgs.SmmFtwFunctionHeader.Function) {
    case FTW_FUNCTION_GET_MAX_BLOCK_SIZE:
      ...
      FtwArgs.SmmGetMaxBlockSizeHeader = (SMM_FTW_GET_MAX_BLOCK_SIZE_HEADER *)FtwArgs.StructureData;

      Status = FtwGetMaxBlockSize (
                 &mFtwDevice->FtwInstance,
                 &FtwArgs.SmmGetMaxBlockSizeHeader->BlockSize
                 );
      break;
    ...
    case FTW_FUNCTION_ALLOCATE:
     ...
     FtwArgs. SmmFtwAllocateHeader = (SMM_FTW_ALLOCATE_HEADER *)FtwArgs.StructureData;
      Status               = FtwAllocate (
                               &mFtwDevice->FtwInstance,
                               &FtwArgs.SmmFtwAllocateHeader->CallerId,
                               FtwArgs.SmmFtwAllocateHeader->PrivateDataSize,
                               FtwArgs.SmmFtwAllocateHeader->NumberOfWrites
                               );
      break;
    ...
    case FTW_FUNCTION_WRITE:
      ...
      FtwArgs.SmmFtwWriteHeader = (SMM_FTW_WRITE_HEADER *)FtwArgs.StructureData;
      Length            = FtwArgs.SmmFtwWriteHeader->Length;
      PrivateDataSize   = FtwArgs.SmmFtwWriteHeader->PrivateDataSize;
      ...
      if (PrivateDataSize == 0) {
        FtwArgs.PrivateData = NULL;
      } else {
        FtwArgs.PrivateData = (VOID *)&FtwArgs.SmmFtwWriteHeader->Data[Length];
      }

      Status = GetFvbByAddressAndAttribute (
                 FtwArgs.SmmFtwWriteHeader->FvbBaseAddress,
                 FtwArgs.SmmFtwWriteHeader->FvbAttributes,
                 &FtwArgs.SmmFvbHandle
                 );
      if (!EFI_ERROR (Status)) {
        //
        ...
        Status = FtwWrite (
                   &mFtwDevice->FtwInstance,
                   FtwArgs.SmmFtwWriteHeader->Lba,
                   FtwArgs.SmmFtwWriteHeader->Offset,
                   Length,
                   FtwArgs.PrivateData,
                   FtwArgs.SmmFvbHandle,
                   FtwArgs.SmmFtwWriteHeader->Data
                   );
      }
    ...
    case FTW_FUNCTION_RESTART:
      ...
      FtwArgs.SmmFtwRestartHeader = (SMM_FTW_RESTART_HEADER *)FtwArgs.StructureData;
      Status              = GetFvbByAddressAndAttribute (
                              FtwArgs.SmmFtwRestartHeader->FvbBaseAddress,
                              FtwArgs.SmmFtwRestartHeader->FvbAttributes,
                              &FtwArgs.SmmFvbHandle
                              );
      if (!EFI_ERROR (Status)) {
        Status = FtwRestart (&mFtwDevice->FtwInstance, FtwArgs.SmmFvbHandle);
      }
    ...
    case FTW_FUNCTION_GET_LAST_WRITE:
      ...
      FtwArgs.SmmFtwGetLastWriteHeader = (SMM_FTW_GET_LAST_WRITE_HEADER *)FtwArgs.StructureData;
      PrivateDataSize          = FtwArgs.SmmFtwGetLastWriteHeader->PrivateDataSize;
      ...
      Status = FtwGetLastWrite (
                 &mFtwDevice->FtwInstance,
                 &FtwArgs.SmmFtwGetLastWriteHeader->CallerId,
                 &FtwArgs.SmmFtwGetLastWriteHeader->Lba,
                 &FtwArgs.SmmFtwGetLastWriteHeader->Offset,
                 &FtwArgs.SmmFtwGetLastWriteHeader->Length,
                 &PrivateDataSize,
                 (VOID *)FtwArgs.SmmFtwGetLastWriteHeader->Data,
                 &FtwArgs.SmmFtwGetLastWriteHeader->Complete
                 );
      FtwArgs.SmmFtwGetLastWriteHeader->PrivateDataSize = PrivateDataSize;
      break;
  ...
  FtwArgs.SmmFtwFunctionHeader.ReturnStatus = Status;
