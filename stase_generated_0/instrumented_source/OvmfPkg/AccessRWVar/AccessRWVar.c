...
/**
  ...
  @retval EFI_INVALID_PARAMETER  The CommBuffer or CommBufferSize parameters are invalid.
  @retval EFI_OUT_OF_RESOURCES   The SMRAM heap does not have sufficient resources to allocate a new
                                 memory pool.
  @retval EFI_BAD_BUFFER_SIZE    The variable offset address is greater than the length of the buffer.
  @retval EFI_INVALID_PARAMATER  The AccessType flag either indicates multiple types or indicates an
                                 unsupported/reserved access type.
**/
EFI_STATUS
EFIAPI
RWVariableHandler (
  ...
  )
{
  ...
  //
  //
  // Verify valid access type (power of 2)
  if ((CommData->AccessType & (CommData->AccessType - 1)) != 0) {
    DEBUG((DEBUG_ERROR, "%a: Invalid access type\n", __func__, CommData->AccessType));
    return EFI_INVALID_PARAMETER;
  }
