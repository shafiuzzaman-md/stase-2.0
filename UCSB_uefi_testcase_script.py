...
output_dir = './UCSB_TESTCASES/UEFI_TESTCASES_STASE'
...
analysis_project_path = f'./UCSB_TESTCASES/UEFI_TESTCASES_STASE/edk2_testcase.project'
...
files.append(f'UEFI_testcase.c')
...
#Zip the results
zipcommand = [
    'zip', '-r', 'UCSB_edk_sa_results.zip', f'{output_dir}'
]
try:
    subprocess.run(zipcommand, check=True)
    print(f"Zipped the results: UCSB_edk_sa_results.zip")
