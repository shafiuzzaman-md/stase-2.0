...
print("Enter the project file location: ")
analysis_project_path = input()
print("Analysis file location: " + analysis_project_path)

print("Reading the filelist.txt")
files = []
...
try:
    ...
    print(f"Error: File '{file_list}' not found.")
    exit()
except IOError as e:
    print(f"I/O error while reading '{file_list}': {e}")
    exit()
except Exception as e:
    print(f"Unexpected error while processing '{file_list}': {e}")
    exit()



# analysis_project_path = f'./KUDU_TESTCASES/kernel_kudu_testcase.project'
...
for file_name in files:
    ...
    main_fact_command += [
        f'{output_path}', '-o', f'{facts_dir_path}'
    ]
...
#Zip the results
zipcommand = [
    'zip', '-r', 'interactive_sa_results.zip', f'{output_dir}'
]
try:
    subprocess.run(zipcommand, check=True)
    print(f"Zipped the results: interactive_sa_results.zip")
