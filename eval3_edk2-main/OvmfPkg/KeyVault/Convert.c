// Convert.c
//
// UCS-2 to ISO-2022-KR conversion implementation supporting Hangul, Hanja, and Symbol mappings.
//

#include "Convert.h"

#define UNKNOWN_10646_CHAR  ((UINT32)0xfffd)
#define ESC  0x1b
#define SI   0x0f
#define SO   0x0e
...
UINTN UCS2ToKSC5601(UINT16 wch, CHAR8 *TmpCh, UINTN avail);
UINTN UCS2ToKSC5601Hangul(UINT16 wch, CHAR8 *TmpCh, UINTN avail);
UINTN UCS2ToKSC5601Hanja(UINT16 wch, CHAR8 *TmpCh, UINTN avail);
UINTN UCS2ToKSC5601Sym(UINT16 wch, CHAR8 *TmpCh, UINTN avail);
  ...
  )
{
  ...
  CHAR8  *BufEnd = Buf + Length;
  CHAR8  *OutbufEnd = Outbuf + OutLength;

  if ((OutbufEnd - Outbuf) < 4) {
  ...
  *Outbuf++ = 'C';
  ...
  enum SET set = ASCII_set;

  while ((CHAR8 *)Buf16 < BufEnd && Outbuf < OutbufEnd) {
    UINT16 ch = *Buf16;
    ch = (ch >> 8) | ((ch & 0x00ff) << 8);  // Endian swap

    // Skip surrogate pairs
    if (((ch & 0xD800) == 0xD800) || ((ch & 0xDC00) == 0xD800)) {
      if ((OutbufEnd - Outbuf) < 2) {
        return EFI_OUT_OF_RESOURCES;
      }
      *Outbuf++ = '\0';
      *Outbuf++ = '\0';
    ...
    if (ch < 0x80) {
      if (set != ASCII_set) {
        if ((OutbufEnd - Outbuf) < 1) {
          return EFI_OUT_OF_RESOURCES;
        }
        *Outbuf++ = SI;
        set = ASCII_set;
      }
      *Outbuf++ = (CHAR8)(ch & 0xff);
      Buf16++;
      continue;
    }

    CHAR8 TmpCh[2];
    UINTN written = UCS2ToKSC5601(ch, TmpCh, 2);
    if (written == UNKNOWN_10646_CHAR || written != 2) {
      return EFI_OUT_OF_RESOURCES;
    }
...
extern const UINT16      __ksc5601_hangul_to_ucs[KSC5601_HANGUL];
  ...
  )
{
  if ((wch >= 0xac00) && (wch <= 0xd7a3)) {
    return UCS2ToKSC5601Hangul(wch, TmpCh, avail);
  } else if (((wch >= 0x4e00) && (wch <= 0x9fff)) || ((wch >= 0xf900) && (wch <= 0xfa0b))) {
    return UCS2ToKSC5601Hanja(wch, TmpCh, avail);
  } else {
    return UCS2ToKSC5601Sym(wch, TmpCh, avail);
  }
}
...
{
  INT32 l = 0;
  INT32 u = KSC5601_HANGUL - 1;

  while (l <= u) {
    INT32 m = (l + u) / 2;
    UINT16 try = __ksc5601_hangul_to_ucs[m];
    ...
    if (try > wch) {
      ...
      if (avail < 2) return 0;
      TmpCh[0] = (CHAR8)((m / 94) + 0x30);
      TmpCh[1] = (CHAR8)((m % 94) + 0x21);
      return 2;
    }
  }
...
{
  INT32 l = 0;
  INT32 u = KSC5601_HANJA - 1;

  while (l <= u) {
    INT32 m = (l + u) / 2;
    UINT16 try = __ksc5601_hanja_from_ucs[m].ucs;
    ...
    if (try > wch) {
      ...
      if (avail < 2) return 0;
      TmpCh[0] = __ksc5601_hanja_from_ucs[m].val[0];
      TmpCh[1] = __ksc5601_hanja_from_ucs[m].val[1];
      return 2;
    }
  }
...
{
  INT32 l = 0;
  INT32 u = KSC5601_SYMBOL - 1;

  while (l <= u) {
    INT32 m = (l + u) / 2;
    UINT16 try = __ksc5601_sym_from_ucs[m].ucs;
    ...
    if (try > wch) {
      ...
      if (avail < 2) return 0;
      TmpCh[0] = __ksc5601_sym_from_ucs[m].val[0];
      TmpCh[1] = __ksc5601_sym_from_ucs[m].val[1];
      return 2;
    }
  }
...
const UINT16  __ksc5601_hangul_to_ucs[KSC5601_HANGUL] =
{
...
const UINT16  __ksc5601_sym_to_ucs[] =
{
