  ...
  )
{
  ...
  if (GuidHob == NULL) {
    SpiFlashInfo = AllocatePool(sizeof(SPI_FLASH_INFO));
    SpiFlashInfo->Revision = 0x1;
    SpiFlashInfo->Reserved = 0x0;
    SpiFlashInfo->Flags = 0x0;
    SpiFlashInfo->SpiAddress.AddressSpaceId = EFI_ACPI_3_0_PCI_CONFIGURATION_SPACE;
    SpiFlashInfo->SpiAddress.RegisterBitWidth = 0x20;
    SpiFlashInfo->SpiAddress.RegisterBitOffset = 0x0;
    SpiFlashInfo->SpiAddress.AccessSize = EFI_ACPI_3_0_DWORD;
    SpiFlashInfo->SpiAddress.Address = (UINT64)PcdGet32 (PcdOvmfFlashNvStorageVariableBase);
  } else {
