...
#define PXEBC_VENDOR_TAG_BOOT_CUSTOM       13
...
#define PXEBC_DHCP4_TAG_INDEX_CUSTOM        7
#define PXEBC_DHCP4_TAG_INDEX_MAX           8
...
#define SET_SINGLE_VENDOR_OPTION_BIT_MAP(x, y, z) \
  (*(x + (*(UINT32 *)(y) / 32)) = (UINT32) ((UINT32) ((x)[*(UINT32 *)(y) / 32]) | BIT ((z) % 32)))
...
typedef struct {
  NETWORK_OBJECT_HEADER Header;
  NETWORK_OBJECT_BODY Body;
} NETWORK_OBJECT;
...
typedef struct {
  ...
  NETWORK_OBJECT* ReplyObject;
  UINT32 TransactionId;
  EFI_STATUS LastOperationStatus;
  NETWORK_DETAILS NetworkInfo;
} PXEBC_BOOT_CUSTOM;
