  ...
  )
{
  ...
  for (PxeOption = (EFI_DHCP4_PACKET_OPTION *)(Dhcp4Option->Data + Offset); 
        (Offset < VendorOptionLen) && (PxeOption->OpCode != DHCP4_TAG_EOP); 
        PxeOption = (EFI_DHCP4_PACKET_OPTION *)(Dhcp4Option->Data + Offset))
  {
    PxeOption = (EFI_DHCP4_PACKET_OPTION *)(Dhcp4Option->Data + Offset);
    if (PxeOption->Length == 0 || PxeOption->Length > VendorOptionLen) {
      break; 
    }
    //
    ...
    switch (PxeOption->OpCode) {
      ...
      case PXEBC_VENDOR_TAG_BOOT_CUSTOM:
        if (PxeOption->Length != sizeof(PXEBC_BOOT_CUSTOM))
          break;
        PXEBC_BOOT_CUSTOM *CBoot = (PXEBC_BOOT_CUSTOM *)&PxeOption->Data[0];

        if (CBoot->ReplyRequested == 1) {
          Status = PxeBcCreateReplyObject(NULL, NULL, CBoot);
          if (Status != EFI_SUCCESS) {
              break;
          }
        }
        PxeBcVendorBootResponse(CBoot);
        VendorOption->BootCustom = CBoot;
        break;
  ...
  //
  // No optcode length, just process Single OpCode.
  // Set the bit map for the special PXE options.
  //
  if (!Iwc)
    SET_SINGLE_VENDOR_OPTION_BIT_MAP (BitMap, Dhcp4Option->Data, PxeOption->OpCode);
}
  ...
  )
{
  ...
  // Cleanup Response.
  // 
  if (IsPxeOffer && (Option != NULL) && (Cache4->VendorOpt.BootCustom->ReplyRequested ==1)) {
    if (Cache4->VendorOpt.BootCustom->ReplyObject->Body.ContentData != NULL) {
      FreePool(Cache4->VendorOpt.BootCustom->ReplyObject->Body.ContentData);
    }
    if (Cache4->VendorOpt.BootCustom->ReplyObject != NULL) {
      FreePool(Cache4->VendorOpt.BootCustom->ReplyObject);
    }
  }
  //
