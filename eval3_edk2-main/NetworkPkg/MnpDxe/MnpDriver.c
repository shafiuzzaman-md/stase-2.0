...
NET_POOL_PRIVATE gEfiNetPoolPublic;
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Could not install Mnp Driver Binding\n"));
    return Status;
  }

  ZeroMem(&gEfiNetPoolPublic, sizeof(NET_POOL_PRIVATE));
  gEfiNetPoolPublic.Signature     = NET_POOL_SIGNATURE;
  gEfiNetPoolPublic.Free          = NULL;
  gEfiNetPoolPublic.Allocate      = NULL;
  gEfiNetPoolPublic.AllocateZero  = NULL;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Could not install network cache\n"));
    return Status;
  }
