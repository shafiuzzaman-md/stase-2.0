  ...
  )
{
  ...
  *NewMessageSize = 0;
  ...
  if ((SeedFieldCount != 0) && (DeleteCount != 0)) {
    TempHeaderFields = HttpAllocateHeaderFields (SeedFieldCount);
  ...
  // Handle AppendList
  //
  NewHeaderFields = HttpAllocateHeaderFields (TempFieldCount + AppendCount);
  ...
  )
{
  ...
  *(TempHttpMessage + HttpMessageSize) = '\0';
  ...
  // Get header number
  //
  HttpUtilitiesCountHeaders(TempHttpMessage, FieldCount);
  ...
  //
  *HeaderFields = HttpAllocateHeaderFields (*FieldCount);
  ...
  while (Index <= *FieldCount) {
    ...
    Status = HttpSetFieldNameAndValue (&(*HeaderFields)[Index], FieldName, FieldValue, Index);
    if (EFI_ERROR (Status)) {
      *FieldCount = 0;
