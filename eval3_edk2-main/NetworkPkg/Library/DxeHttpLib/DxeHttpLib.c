  ...
  )
{
  ...
  if (HttpHeader->FieldName != NULL) {
    HttpHeader->FieldName = NULL;
  }

  if (HttpHeader->FieldValue != NULL) {
    HttpHeader->FieldValue = NULL;
  }

  FieldNameSize         = AsciiStrSize (FieldName);
  HttpHeader->FieldName = AllocateZeroNetPool (FieldNameSize);
  if (HttpHeader->FieldName == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  HttpHeader->FieldValue = AllocateZeroNetPool (FieldValueSize);
  if (HttpHeader->FieldValue == NULL) {
  ...
  HttpHeader->Index = Index;

  return EFI_SUCCESS;
}

/**
  Count key-value pairs in a raw HTTP header block.

  @param[in]      HttpMessage         Raw HTTP header to count from.
  @param[in,out]  FieldCount          The number of fields counted.

**/
VOID
EFIAPI
HttpUtilitiesCountHeaders (
  ...
  *FieldCount   = 0;
  ...
  while (TRUE) {
    ...
    *(FieldValueStr - 1) = 0;
    ...
    //
    // Handle FieldValueStr, skip all the preceding LWS.
    //
    while (TRUE) {
    ...
    //
    // Handle FieldValueStr, verify LWS within field value
    //
...
/**
  Allocate an array of HeaderFields.

  @param[in]  FieldCount         The number of header pairs in HeaderFields.

  @return     Pointer to the HeaderFields array.
  @return     NULL if the allocation fails.

**/
EFI_HTTP_HEADER*
EFIAPI
HttpAllocateHeaderFields (
  IN UINTN  FieldCount
  )
{
  EFI_HTTP_HEADER *Headers;

  Headers = AllocateZeroNetPool ((FieldCount) * sizeof (EFI_HTTP_HEADER));
  if (Headers == NULL) {
    return (VOID *)NULL;
  }
  ...
  if (HeaderFields != NULL) {
    for (Index = 0; Index < FieldCount; Index++) {
      if (HeaderFields[Index].FieldName != NULL) {
        FreeNetPool (HeaderFields[Index].FieldName);
      }

      if (HeaderFields[Index].FieldValue != NULL) {
        FreeNetPool (HeaderFields[Index].FieldValue);
      }
    }

    FreeNetPool (HeaderFields);
  }
}
